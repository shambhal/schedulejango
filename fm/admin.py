...
class FaltuAdmin(admin.ModelAdmin):
    ...
    def deleteF(self, request):
        ...
        for f1 in path1:
            ...
            if p.is_file():
                ...
                #print("This is dir")
                shutil.rmtree(p)   # works even if directory is NOT empty
    ...
    def upload_fm(self, request):
        ...
        if directory != None and directory != "":
            pdirectory = settings.MEDIA_ROOT + "catalog/" + directory+'/'
            prefix = "catalog/" +directory+'/'

        else:
            pdirectory = settings.MEDIA_ROOT + "catalog/"
            prefix = "catalog/"
